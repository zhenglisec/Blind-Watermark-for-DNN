...
from models import SSIM, DPN26
from models.Discriminator import DiscriminatorNet1
...
parser.add_argument('--dataroot', default='/data/lizheng/DATASET/CIFAR-10')
parser.add_argument('--train', type=bool, default=True)
parser.add_argument('--num_epochs', type=int, default=100)
parser.add_argument('--batchsize', type=int, default=100)
parser.add_argument('--wm_num', default=500, type=int,
                    help='the number of wm images')
parser.add_argument('--wm_batchsize', default=20,
                    type=int, help='the wm batch size')
parser.add_argument('--lr', type=float, default=0.001)
parser.add_argument('--alpha', '--list', nargs='+', default=[1])
parser.add_argument('--beta', '--list1', nargs='+',
                    default=[3, 5, 1, 0.1])  # 在暂时是最优
...
if args.arch.find('vgg')!= -1:
    ...
    model.classifier[3] = nn.L[tensor([6, 9], device='cuda:0'), tensor([9, 4], device='cuda:0'), tensor([1, 1], device='cuda:0'), tensor([2, 7], de
    model.classifier[6] = nn.L[tensor([6, 9], device='cuda:0'), tensor([9, 4], device='cuda:0'), tensor([1, 1], device='cuda:0'), tensor([2, 7], de
elif args.arch.find('resnet')![tensor([6, 9], device='cuda:0'), tensor([9, 4], device='cuda:0'), tensor([1, 1], device='cuda:0'), tensor([2, 7], de
    model.fc = nn.Linear(2028,[tensor([6, 9], device='cuda:0'), tensor([9, 4], device='cuda:0'), tensor([1, 1], device='cuda:0'), tensor([2, 7], de
print(model)

if torch.cuda.is_available():
    cudnn.benchmark = True
    if args.seed is not None:
        random.seed(args.seed)[tensor([6, 9], device='cuda:0'), tensor([9, 4], device='cuda:0'), tensor([1, 1], device='cuda:0'), tensor([2, 7], de
...
# 加载触发集
#triggerset = torchvision.datasets.ImageFolder(root=args.dataroot+'/triggerset_from_train', transform=transform_test)
...
#trigger_testset = torchvision.datasets.CIFAR10(root=args.dataroot, train=True, download=True, transform=transform_test)
#trigger_testloader = torch.utils.data.DataLoader(trigger_testset, batch_size=args.wm_batchsize, shuffle=False, num_workers=2, drop_last=True)
# 加载logo
...
if args.wm_train:
    for wm_idx, (wm_input, wm_cover_label) in enumerate(trigger_loader):
        ...
        # wm_labels.append(SpecifiedLabel(wm_target))
        if wm_idx == (int(args.wm_num/args.wm_batchsize)-1):  # 训练集 1% 的部分作触发集
            break
'''
wm_testinputs, wm_testtargets = [], []
if args.wm_train:
    for wm_idx, (wm_input, wm_target) in enumerate(trigger_testloader):
        wm_input, wm_target = wm_input.cuda(), wm_target.cuda()
        wm_testinputs.append(wm_input)
        wm_testtargets.append(wm_target)
        #wm_labels.append(SpecifiedLabel(wm_target))
        if wm_idx == 2:# 训练集 1% 的部分作触发集
            break

wm_idx = np.random.randint(len(wm_inputs))
'''
#print(wm_cover_labels, wm_testtargets)
...
np_labels = np.random.randint(
    10, size=(int(args.wm_num/args.wm_batchsize), args.wm_batchsize))
...
schedulerD = ReduceLROnPlateau(
    optimizerD, mode='min', factor=0.2, patience=8, verbose=True)

criterionN = nn.CrossEntropyLoss()
optimizerN = optim.SGD(Dnnet.parameters(), lr=0.1,
...
def train(epoch):
    ...
    for batch_idx, (input, label) in enumerate(trainloader):
        ...
        loss_H = args.beta[0] * loss_mse + args.beta[1] * \
            (1 - loss_ssim) + args.beta[2] * loss_adv + args.beta[3] * loss_dnn
        loss_H.backward()
        optimizerH.step()
        ################优化 DNNet#############
        optimizerN.zero_grad()
        ...
        print('[%d/%d][%d/%d]  Loss D: %.4f () Loss_H: %.4f (mse: %.4f ssim: %.4f adv: %.4f)  Loss_real_DNN: %.4f Real acc: %.3f  wm acc: %.3f wm cover acc: %.3f GPU: %d' % (
            ...
            100. * real_correct / real_total, 100. * wm_correct / wm_total, 100. * wm_cover_correct / wm_total, int(GPU)))
...
def save_loss_acc(epoch, loss, acc, train):
    ...
    ax1.set_xlabel('Epoch(' + ",".join(str(i) for i in args.alpha) +
                   '|' + ",".join(str(l) for l in args.beta) + ')')
